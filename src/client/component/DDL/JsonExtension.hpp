...
namespace nlohmann
{
    ...
    template<class T> void optional_to_json(nlohmann::ordered_json& j, const char* name, const std::optional<T>& value)
    {
        if (value)
            j[name] = *value;
    }

    template<class T> void optional_from_json(const nlohmann::ordered_json& j, const char* name, std::optional<T>& value)
    {
        const auto it = j.find(name);
        if (it != j.end() && !it->is_null())
            value = it->get<T>();
    ...
    template<typename T> void extended_to_json(const char* key, nlohmann::ordered_json& j, const T& value)
    {
        if constexpr (is_optional<T>)
            nlohmann::optional_to_json(j, key, value);
        else
    ...
    template<typename T> void extended_from_json(const char* key, const nlohmann::ordered_json& j, T& value)
    {
        if constexpr (is_optional<T>)
            nlohmann::optional_from_json(j, key, value);
        else
...
#define NLOHMANN_DEFINE_TYPE_EXTENSION_ORDERED(Type, ...)                                                                                                              \
